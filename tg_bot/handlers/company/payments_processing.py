...
class PaymentsProcessing:
    ...
    async def stripe(callback: types.CallbackQuery, state: FSMContext):
        ...
        result = await DbPayment(
            creator_id=uid, amount=driver.form_price, p_type=PaymentsProcessing.PAY_FOR_DRIVER,
            driver_id=current_driver_id, invoice_url=invoice_url, status=0, system=PaymentsProcessing.STRIPE,
            ...
            for payment in db_payments:
                ...
                if invoice_obj.status == "paid":
                    ...
                    if payment.type == PaymentsProcessing.PAY_FOR_DRIVER:
                        msg_key = "pay_for_driver_success"
                    ...
                    text = await Ut.get_message_text(lang=company.lang, key=msg_key)
